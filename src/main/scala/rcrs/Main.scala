...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    val firesCount = args(0).toInt
    val brigadesCount = args(1).toInt

    val argsWithoutBrigadesAndFires = args.drop(2)

    val config = new Config
    val restArgs = CommandLineOptions.processArgs(argsWithoutBrigadesAndFires, config)
...
//      launcher.connect(new SimpleFireBrigadeAgent().rcrsAgent)
    ...
    launcher.connect(new CentralAgent(firesCount).rcrsAgent)
